...
CreateThread(function()
    while true do
        Wait(500)
        for k,v in pairs(Config.PostalLocations) do
            local playerCoords = GetEntityCoords(cache.ped)
            local distance = #(playerCoords - v.npccoords.xyz)

            if distance < Config.DistanceSpawn and not spawnedPeds[k] then
                local spawnedPed = NearPed(v.npcmodel, v.npccoords, v.location, v.coords)
...
function NearPed(npcmodel, npccoords, location, coords)
        ...
        options = {
            {
                ...
                label = 'Postal Service Menu',
                targeticon = 'fa-solid fa-eye',
                action = function()
                    TriggerEvent('rex-postal:client:openpostal', location, coords)
                end
            },
        },
        distance = 2.0,
